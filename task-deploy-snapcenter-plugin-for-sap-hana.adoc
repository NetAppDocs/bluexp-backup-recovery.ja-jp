...
. 次の場所にある* sudoer.txt *の内容をコピーします。_/var/lib/docker/volumes/service-manager [1]-2_cloudmanager_SCS_cloud_volume/_data/$（sudo docker ps | grep -Po "cloudmanager_SCS_cloud：.*？"|sed -e 's/*$//'| cut-f2-d"："）/sc-linux-host-plugin_
...
.. プラグインホストを選択します。
（オプション）ホストが追加されていない場合は、*[プラグインホストを追加]*をクリックします。
