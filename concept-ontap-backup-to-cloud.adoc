...
* オンプレミスのONTAP システムでは、フルボリュームリストアがサポートされています。Cloud Volumes ONTAP システムは現在サポートされていません。
* ファイルレベルのリストアは、オンプレミスのONTAP システムとCloud Volumes ONTAP システムの両方でサポートされています。
* ディレクトリ/フォルダのリストアは現在サポートされていません。
* バックアップをアーカイブストレージに移動できません。
* バックアップではDataLockとRansomwareによる保護は使用できません。
...
* 異なるサブネットに異なるBlueXPシステムを持つ同じアカウントを使用している場合、ファイルレベルの復元はサポートされません。
