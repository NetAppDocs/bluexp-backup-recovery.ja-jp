...
+
--
* Cloud ManagerでデスティネーションAWSアカウントのクレデンシャルを追加します
* Cloud Managerに権限を付与するIAMロールに「s3：PutBucketPolicy」という権限を追加します
...
AWS 権限が必要です:: Cloud Manager に権限を提供する IAM ロールが必要です 最新の S3 権限を含める https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions-aws.html["Cloud Manager ポリシー"^]。
+
--
ポリシーの具体的な権限を次に示します。
...
+
バックアップに別のAWSアカウントを使用する場合は、Cloud ManagerでデスティネーションAWSアカウントのクレデンシャルを追加し、Cloud Managerに権限を提供するIAMロールに「s3：PutBucketPolicy」権限を追加する必要があります。
