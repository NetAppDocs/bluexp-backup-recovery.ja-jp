...
[role="lead"]
BlueXPのバックアップとリカバリ（Cloud Backup）は、オンプレミスとクラウドの両方で、NetApp ONTAP データ、Kubernetesの永続ボリューム、データベース、仮想マシンに、効率的でセキュアな対費用効果の高いデータ保護を提供します。バックアップは自動的に生成され、パブリックまたはプライベートクラウドアカウントのオブジェクトストアに格納されます。
...
TIP: BlueXP Connectorがクラウドの行政機関のリージョンや、インターネットアクセスのないサイト（ダークサイト）に導入されている場合、ONTAP システムからのバックアップとリストアの処理のみがサポートされます。このような導入方法を使用する場合、Cloud BackupはKubernetesクラスタ、アプリケーション、または仮想マシンからのバックアップとリストアの処理をサポートしません。
