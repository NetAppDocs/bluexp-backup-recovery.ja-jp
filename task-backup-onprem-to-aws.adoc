...
独自のバケットを作成する場合は、「netapp-backup」で始まるバケット名を使用する必要があります。カスタムの名前を使用する必要がある場合は、[Setup S3 permissions]セクションで使用するポリシーを編集して、独自のカスタムバケットプレフィックスに一致するリソースARNを使用します。
