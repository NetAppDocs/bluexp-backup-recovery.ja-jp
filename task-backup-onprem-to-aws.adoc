...
[NOTE]
====
AWS GovCloudまたはAWS Chinaリージョンでバックアップを作成する場合は、次の2つの権限セットのall_Resource_sectionsの下にあるAWSリソース名「arn」を変更する必要があります。

* AWS GovCloudリージョンの場合は、「aws」を「aws-us-gov」に変更する必要があります。例： `arn:aws-us-gov:s3:::netapp-backup-*`。
* AWS Chinaリージョンの場合は、「aws」を「aws-cn」に変更する必要があります。例： `arn:aws-cn:s3:::netapp-backup-*`。
...
独自のバケットを作成する場合は、バケット名として「netapp-backup」を使用する必要があります。カスタム名を使用する必要がある場合は、を編集します `ontapcloud-instance-policy-netapp-backup` 既存のCVOにIAMRoleを割り当て、S3権限に次のリストを追加します。
...
[NOTE]
====
カスタムバケットがAWS GovCloudまたはAWS Chinaリージョンにある場合は、権限のall_Resource_sectionsの下にあるAWSリソース名「arn」を変更する必要があります。

* AWS GovCloudリージョンの場合は、「aws」を「aws-us-gov」に変更する必要があります。例： `arn:aws-us-gov:s3:::*`。
* AWS Chinaリージョンの場合は、「aws」を「aws-cn」に変更する必要があります。例： `arn:aws-cn:s3:::-*`。
...
標準のパブリックインターネット接続を使用する場合は、すべてのアクセス権がコネクタによって設定され、他に必要な操作はありません。このタイプの接続がに表示されます link:task-backup-onprem-to-aws.html#identify-the-connection-method["最初のダイアグラム"]。

オンプレミスのデータセンターからVPCへのインターネット接続をよりセキュアにする場合は、バックアップアクティブ化ウィザードでAWS PrivateLink接続を選択できます。VPNまたはAWS Direct Connectを使用して、プライベートIPアドレスを使用するVPCエンドポイントインターフェイス経由でオンプレミスシステムに接続する場合は、この環境が必要です。このタイプの接続がに表示されます link:task-backup-onprem-to-aws.html#identify-the-connection-method["2番目の図"]。
...
. *ローカルSnapshot *：既存のSnapshotポリシーを選択するか、ポリシーを作成します。
+
