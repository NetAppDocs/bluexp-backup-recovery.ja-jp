...
コネクタがクラウドに導入されている場合、Cloud BackupはGovCloudリージョンでサポートされます。オンプレミスまたはダークサイトにインストールされている場合はサポートされません。また、AWS MarketplaceからConnectorを導入する必要があります。Connectorは、Cloud Manager SaaS Webサイトからは政府機関の地域には導入できません。 https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-launching-aws-mktp.html#create-the-connector-in-an-aws-government-region["AWS GovCloudリージョンでコネクタを作成する方法について説明します"]。
...
. Cloud Manager Connector に関連付けられているセキュリティグループの設定を変更します。このポリシーを「 Custom 」（「 Full Access 」から）に変更する必要があります。また、変更する必要があります  up S3 permissions,バックアップポリシーから S3 権限を追加します 前に示したように、
+
