...
コネクタがクラウドに導入されている場合、Cloud BackupはGovCloudリージョンでサポートされます。オンプレミスまたはダークサイトにインストールされている場合はサポートされません。また、AWS MarketplaceからConnectorを導入する必要があります。政府機関の地域では、BlueXP SaaS WebサイトからConnectorを展開することはできません。 https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-launching-aws-mktp.html#create-the-connector-in-an-aws-government-region["AWS GovCloudリージョンでコネクタを作成する方法について説明します"]。
