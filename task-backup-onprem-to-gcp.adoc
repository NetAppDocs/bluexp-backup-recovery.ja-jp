...
[role="quick-margin-para"]
必要に応じて、Google Cloudのデフォルトの暗号化キーを使用する代わりに、データ暗号化用に独自のカスタム管理キーを設定できます。 <<Prepare Google Cloud storage as your backup target,Google Cloud環境でONTAPバックアップを受信できるようにする方法をご紹介します。>>
