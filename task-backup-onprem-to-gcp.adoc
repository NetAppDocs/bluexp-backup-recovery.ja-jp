...
+
Private Google AccessまたはPrivate Service Connectを使用している場合は、DNSサーバーがstorage.googleapis.comを正しい内部（プライベート）IPアドレスに指定するように設定されていることを確認します。
...
+
Googleの指示に従って、プライベートアクセスオプションを設定します。DNSサーバが、www.googleapis.comおよびstorage.googleapis.comを正しい内部（プライベート）IPアドレスに指すように設定されていることを確認します。
