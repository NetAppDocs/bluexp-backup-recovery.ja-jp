...
独自のストレージアカウントを作成する場合は、「netapp-backup」というストレージアカウント名を使用する必要があります。
...
* 選択するボリュームのSnapLock設定も同じである必要があります。すべてのボリュームでSnapLock Enterpriseが有効になっているかSnapLockが無効になっている必要があります。（SnapLock準拠モードのボリュームは現在サポートされていません）。 ロックされているボリュームとロックされていないボリュームの組み合わせは選択できません。
